...
from auto_mixer.modules import TextHyperMixer, TwoLayeredPerceptron
...
class AbstractMMImdbMixer(AbstractTrainTestModule, ABC):
    ...
    def setup_scores(self) -> list[dict[str, Metric] | dict[str, Metric] | dict[str, Metric]]:
        ...
        val_scores = self.get_scores()
        test_scores = self.get_scores()
        return [train_scores, val_scores, test_scores]

    def get_scores(self):
        TASK = "multilabel"
        return dict(accuracy=Accuracy(task=TASK, num_labels=NUM_LABELS),
                    recall_macro=Recall(task=TASK, num_labels=NUM_LABELS, average='macro'),
                    recall_micro=Recall(task=TASK, num_labels=NUM_LABELS, average='micro'),
                    precision_macro=Precision(task=TASK, num_labels=NUM_LABELS, average='macro'),
                    precision_micro=Precision(task=TASK, num_labels=NUM_LABELS, average='micro'),
                    f1_macro=F1Score(task=TASK, num_labels=NUM_LABELS, average='macro'),
                    f1_micro=F1Score(task=TASK, num_labels=NUM_LABELS, average='micro'),
                    f1_weighted=F1Score(task=TASK, num_labels=NUM_LABELS, average='weighted'))

    def configure_optimizers(self):
        optimizer_cfg = self.optimizer_cfg
        optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, self.parameters()), **optimizer_cfg)
...
class MMImdbImageMixer(AbstractMMImdbMixer):
    def __init__(self, model_cfg: DictConfig, optimizer_cfg: DictConfig, **kwargs):
        super(AbstractMMImdbMixer, self).__init__(optimizer_cfg, **kwargs)
        mixer_cls = getattr(modules, model_cfg.modalities.block_type)
        self.backbone = mixer_cls(**model_cfg.modalities.image, dropout=model_cfg.dropout)
        self.classifier = TwoLayeredPerceptron(**model_cfg.modalities.classification)

    def shared_step(self, batch, **kwargs):
        image = batch['image']
        labels = batch['label']
        image_logits = self.get_logits(image)
...
class MMImdbTextEmbeddingMixer(AbstractMMImdbMixer):
    def __init__(self, model_cfg: DictConfig, optimizer_cfg: DictConfig, **kwargs):
        ...
        self.classifier = torch.nn.Linear(model_cfg.modalities.text.patch_size,
                                          model_cfg.modalities.classification.num_classes)
...
class MMImdbAveragedTextEmbeddingMixer(AbstractMMImdbMixer):
    def __init__(self, model_cfg: DictConfig, optimizer_cfg: DictConfig, **kwargs):
        super(AbstractMMImdbMixer, self).__init__(optimizer_cfg, **kwargs)
        model_cls = getattr(modules, model_cfg.modalities.block_type)
        self.model = model_cls(**model_cfg.modalities.text, dropout=model_cfg.dropout)
...
class MultiBenchTextMixer(AbstractMMImdbMixer):
    def __init__(self, model_cfg: DictConfig, optimizer_cfg: DictConfig, **kwargs):
        super(AbstractMMImdbMixer, self).__init__(optimizer_cfg, **kwargs)
        self.model = TwoLayeredPerceptron(**model_cfg.modalities.text)
        self.classifier = TwoLayeredPerceptron(**model_cfg.modalities.classification)
...
class MMImdbVggEmbeddingsMixer(AbstractMMImdbMixer):
    def __init__(self, model_cfg: DictConfig, optimizer_cfg: DictConfig, **kwargs):
        super(AbstractMMImdbMixer, self).__init__(optimizer_cfg, **kwargs)
        model_cls = getattr(modules, model_cfg.modalities.block_type)
        self.model = model_cls(**model_cfg.modalities.features, dropout=model_cfg.dropout)
        self.classifier = TwoLayeredPerceptron(**model_cfg.modalities.classification)
