...
class TextMLPMixer(nn.Module):
    def __init__(self,
        ...
        for _ in range(num_mixers):
            self.mixer_blocks.append(
                MixerBlock(hidden_dim, self.num_patch, channel_dim, num_heads, dropout=dropout))
        self.layer_norm = nn.LayerNorm(hidden_dim)
...
if __name__ == "__main__":
    ...
    model = MLPMixer(in_channels=3, image_size=[224, 224], patch_size=32, num_classes=1000,
                     hidden_dim=512, num_mixers=9, token_dim=256, channel_dim=2048)
