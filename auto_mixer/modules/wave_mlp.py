"""
This module provides an implementation of the Wave-MLP architecture.

Key Features:
- Dynamic Token Aggregation: Tokens are represented as wave functions with amplitude and phase,
 allowing dynamic aggregation based on image semantics.
- Wave-like Token Representation: Each token has an amplitude (original feature) and a phase term (complex value),
 enhancing relationships between tokens.

 Source: https://arxiv.org/pdf/2111.12294.pdf
"""
...
def basic_blocks(dim, index, layers, mlp_ratio=3., qkv_bias=False, qk_scale=None, attn_drop=0.,
                 drop_path_rate=0., norm_layer=nn.BatchNorm2d, mode='fc', **_kwargs):
    blocks = []
...
class WaveMLP(nn.Module):
    def __init__(self, layers, patch_size=7, in_chans=3, num_classes=1000,
...
def wave_mlp_t_dw(**kwargs):
    transitions = [True, True, True, True]
    layers = [2, 2, 4, 2]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
    model = WaveMLP(layers, embed_dims=embed_dims, patch_size=7, transitions=transitions,
                    mlp_ratios=mlp_ratios, mode='depthwise', **kwargs)
    model.default_cfg = default_cfgs['wave_T']
    return model


def wave_mlp_t(**kwargs):
    transitions = [True, True, True, True]
    layers = [2, 2, 4, 2]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
...
def wave_mlp_s(**kwargs):
    transitions = [True, True, True, True]
    layers = [2, 3, 10, 3]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
    model = WaveMLP(layers, embed_dims=embed_dims, patch_size=7, transitions=transitions,
                    mlp_ratios=mlp_ratios, norm_layer=my_norm, **kwargs)
    model.default_cfg = default_cfgs['wave_S']
    return model


def wave_mlp_m(**kwargs):
    transitions = [True, True, True, True]
    layers = [3, 4, 18, 3]
    mlp_ratios = [8, 8, 4, 4]
    embed_dims = [64, 128, 320, 512]
    model = WaveMLP(layers, embed_dims=embed_dims, patch_size=7, transitions=transitions,
                    mlp_ratios=mlp_ratios, norm_layer=my_norm, ds_use_norm=False, **kwargs)
    model.default_cfg = default_cfgs['wave_M']
    return model


def wave_mlp_b(**kwargs):
    transitions = [True, True, True, True]
    layers = [2, 2, 18, 2]
    mlp_ratios = [4, 4, 4, 4]
...
class PretrainedFrozenWaveMLP(nn.Module):
    def __init__(self, weights_path, **_kwargs):
        super().__init__()
        self.model = load_pretrained_model(wave_mlp_t_dw(),
                                           weights_path)
...
class UnfrozenPretrainedWaveMLP(nn.Module):
    def __init__(self, weights_path, **_kwargs):
        super().__init__()
        self.model = load_pretrained_model(wave_mlp_t_dw(),
                                           weights_path)
